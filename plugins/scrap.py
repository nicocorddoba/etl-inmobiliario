...
def pagination(base_url, chromium, logger):
    # Lanzar navegador para obtener número de páginas
    context, browser = launch_browser(chromium)
    ...
    try:
        ...
        number_of_pages = int(page.locator("li.pagination__page").nth(-2).inner_text())
    except Exception as e:
        logger.error("Error: No se pudo cargar la página para contar páginas")
        raise
    finally:
        page.close()
        browser.close()
...
def scrap_propertie_details(url: str, page: Page, logger):
    try:
        ...
        logger.error(f"Error: No se pudo cargar la página {url}: {e}")
        raise
...
def run(url: str, logger=None, province: str = "tucuman") -> list[dict]:
    ...
    logger.info(f"Scraping completed, found {len(data)} properties in {province}")
    return data